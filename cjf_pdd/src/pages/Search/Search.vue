...
<script>
import SearchNav from './children/serachnav'
...
export default {
  ...
  computed:{
      ...mapState(['searchgoods']),
        //用于动态决定左侧哪个选项被选中
      currentIndex() {
        const {scrollY, rightLiTops} = this;// 获取数据
        // 取出索引
  ...
  watch: {
      searchgoods() {
        this.$nextTick(() => {
          this._initBScroll();//初始化
          this._initRightLiTops(); // 求出右边所有版块的头部位置
        })
      }
    },
    methods: {
