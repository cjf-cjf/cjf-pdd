<template>
            <!--<a class="recommend-item">
                <img src="./imgs/shop_item.png" alt="" width="100%">
               <h4 class="list-item-title">【唐幂】整条民族风围巾女披肩保暖</h4>
               <div class="list-item-bottom">
                   <span class="item-price">￥26.9</span>
                   <span class="item-counter">已拼546件</span>
                   <button class="item-btn">找相关</button>
               </div>
            </a>-->
            <a class="recommend-item">
                <img :src="item.thumb_url" alt="" width="100%" v-if="item.thumb_url">
               <h4 class="list-item-title">{{item.short_name}}</h4>
               <div class="list-item-bottom">
                   <span class="item-price">￥{{item.price /100}}</span>
                   <span class="item-counter">{{item.sales_tip}}</span>
                   <button class="item-btn">找相关</button>
               </div>
            </a>
</template>
<script>
export default {
    name:"shoplist",
    props:{
      item:Object
    }
}
</script>
<style scoped lang="stylus" ref="stylesheet/stylus">
      .recommend-item:nth-child(2n-1)
        margin-right 1%
      .recommend-item
        width 49.5%
        background #FFF
        padding-bottom 15px
        margin-bottom 15px
        list-style none
        .list-item-title
          line-height 20px
          font-size 13px
          font-weight lighter
          height 20px
          overflow hidden
          margin 5px 0
          padding 0 5px
        .list-item-bottom
          display flex
          flex-direction row
          align-items center
          padding 0 6px
          .item-price
            flex 2
            color red
            font-weight bolder
            font-size 12px
          .item-counter
            flex 4
            font-size 10px
            color #666666
          .item-btn
            flex 2
            border 1px solid orangered
            height 26px
            border-radius 5px
            font-size 10px
            background-color transparent
            color red
</style>